...
/**
 * An isolated {@link ClassLoader} for running all user code. All user files are compiled from source code byte a
 * {@link ByteCodeCompiler}. After compilation, some byte code rewriting is performed to support
 * <code>JavaScriptObject</code> and its subtypes.
 * 
 * TODO: we should refactor this class to move the getClassInfoByDispId, getDispId, getMethodDispatch and
 * putMethodDispatch into a separate entity since they really do not interact with the CompilingClassLoader
 * functionality.
 */
public final class CompilingClassLoader extends ClassLoader implements
  ...
  /**
   * Oracle that can answer questions about {@link DispatchClassInfo DispatchClassInfos}.
   */
  private final class DispatchClassInfoOracle {
    ...
    /**
     * Returns the dispatch id for a given member reference. Member references can be encoded as: "@class::field" or
    ...
    /**
     * Returns the {@link java.lang.Class} instance for a given binary class name. It is important to avoid initializing
     * the class because this would potentially cause initializers to be run in a different order than in web mode.
     * Moreover, we may not have injected all of the JSNI code required to initialize the class.
     * 
     * @param binaryClassName
     *          the binary name of a class
     * @return {@link java.lang.Class} instance or null if the given binary class name could not be found
     */
    ...
    /**
     * Returns the {@link java.lang.Class} object for a class that matches the source or binary name given.
     * 
     * @param className
    ...
    /**
     * Returns the {@link DispatchClassInfo} associated with the class name. Since we allow both binary and source names
     * to be used in JSNI class references, we need to be able to deal with the fact that multiple permutations of the
     * class name with regards to source or binary forms map on the same {@link DispatchClassInfo}.
     * 
     * @param className
     *          binary or source name for a class
     * @return {@link DispatchClassInfo} associated with the binary or source class name; null if there is none
     */
  ...
  /**
   * A ClassLoader that will delegate to a parent ClassLoader and fall back to loading bytecode as resources from an
   * alternate parent ClassLoader.
   */
  ...
  /**
   * Implements {@link InstanceMethodOracle} on behalf of the {@link HostedModeClassRewriter}. Implemented using
   * {@link TypeOracle}.
   */
  private class MyInstanceMethodOracle implements InstanceMethodOracle {
    ...
    /**
     * Record that a given JSO type contains the concrete implementation of a (possibly abstract) method.
     */
  ...
  /**
   * Cook up the data we need to support JSO subtypes that implement interfaces with methods. This includes the set of
   * SingleJsoImpl interfaces actually implemented by a JSO type, the mangled method names, and the names of the Methods
   * that should actually implement the virtual functions.
   * 
   * Given the current implementation of JSO$ and incremental execution of rebinds, it's not possible for Generators to
   * produce additional JavaScriptObject subtypes, so this data can remain static.
   */
  private class MySingleJsoImplData implements SingleJsoImplData {
    ...
    private final Map<String, List<com.google.gwt.dev.asm.commons.Method>> mangledNamesToDeclarations =
        new HashMap<String, List<com.google.gwt.dev.asm.commons.Method>>();
    private final Map<String, List<com.google.gwt.dev.asm.commons.Method>> mangledNamesToImplementations =
        new HashMap<String, List<com.google.gwt.dev.asm.commons.Method>>();
    ...
    /**
     * Looks for a concrete implementation of <code>intfMethod</code> in <code>implementingType</code>.
     */
  ...
  /**
   * The set of classes exposed into user space that actually live in hosted space (thus, they bridge across the
   * spaces).
   */
  ...
  /**
   * Magic: {@link JavaScriptHost} was never compiled because it's a part of the hosted mode infrastructure. However,
   * unlike {@link #BRIDGE_CLASSES}, {@code JavaScriptHost} needs a separate copy per inside the ClassLoader for each
   * module.
   */
   ...
   */
  private final Set<CompilationUnit> alreadyInjected = new HashSet<CompilationUnit>();
   ...
   * Used by {@link #findClass(String)} to prevent reentrant JSNI injection.
   */
  private final Stack<CompilationUnit> toInject = new Stack<CompilationUnit>();
  ...
  /**
   * Retrieves the mapped JSO for a given unique id, provided the id was previously cached and the JSO has not been
   * garbage collected.
   * 
   * @param uniqueId
   *          the previously stored unique id
   * @return the mapped JSO, or <code>null</code> if the id was not previously mapped or if the JSO has been garbage
   *         collected
   */
   ...
   * @param dispId
   *          dispatch identifier
   * @return {@link DispatchClassInfo} for a given dispatch id or null if one does not exist
   */
  @Override
  ...
  /**
   * Retrieves the mapped wrapper for a given Java Object, provided the wrapper was previously cached and has not been
   * garbage collected.
   * 
   * @param javaObject
   *          the Object being wrapped
   * @return the mapped wrapper, or <code>null</code> if the Java object mapped or if the wrapper has been garbage
   *         collected
   */
  ...
  /**
   * Weakly caches a given JSO by unique id. A cached JSO can be looked up by unique id until it is garbage collected.
   * 
   * @param uniqueId
  ...
  protected synchronized Class<?> findClass(String className)
    ...
    if (!className.contains("shrinkwrap")) {
  ...
  /**
   * Returns the compilationUnit corresponding to the className. For nested classes, the unit corresponding to the top
  ...
  /**
   * Tricky one, this. Reaches over into this modules's JavaScriptHost class and sets its static 'host' field to our
   * module space.
   * 
   * @see JavaScriptHost
   */
