 ...
 * 
 * <p>
 * The test environment consists of this class and the non-translatable version of
 * {@link com.google.gwt.junit.client.GWTTestCase}. These two classes integrate directly into the real JUnit test
 * process.
 * </p>
 * 
 * <p>
 * The client classes consist of the translatable version of {@link com.google.gwt.junit.client.GWTTestCase},
 * translatable JUnit classes, and the user's own {@link com.google.gwt.junit.client.GWTTestCase}-derived class. The
 * client communicates to the server via RPC.
 * </p>
 * 
 * <p>
...
public class JUnitShell extends DevMode {
  ...
  private final class MyJettyLauncher extends JettyLauncher {
    ...
    /**
     * Ignore DevMode's normal WEB-INF classloader rules and just allow the system classloader to dominate. This makes
     * JUnitHostImpl live in the right classloader (mine).
     */
  ...
  /**
   * Singleton object for hosting unit tests. All test case instances executed by the TestRunner will use the single
   * unitTestShell.
   */
  ...
  /**
   * Checks if a testCase should not be executed. Currently, a test is either executed on all clients (mentioned in this
   * test) or on no clients.
   * 
   * @param testInfo
   *          the test info to check
   * @return true iff the test should not be executed on any of the specified clients.
   */
  ...
  public static void runTest(GWTTestCase testCase, Class<?> testClass, TestResult testResult)
      throws UnableToCompleteException {
    getUnitTestShell().runTestImpl(testCase, testClass, testResult);
  }

  /**
   * Entry point for {@link com.google.gwt.junit.client.GWTTestCase}. Gets or creates the singleton {@link JUnitShell}
   * and invokes its {@link #runTestImpl(GWTTestCase, TestResult)}.
   */
  public static void runTest(GWTTestCase testCase, TestResult testResult)
      throws UnableToCompleteException {
    getUnitTestShell().runTestImpl(testCase, null, testResult);
  }
  ...
  /**
   * Sanity check; if the type we're trying to run did not actually wind up in the type oracle, there's no way this test
   * can possibly run. Bail early instead of failing on the client.
   */
  private static JUnitFatalLaunchException checkTestClassInCurrentModule(TreeLogger logger,
      CompilationState compilationState, String moduleName, TestCase testCase, Class<?> testClass) {
    TypeOracle typeOracle = compilationState.getTypeOracle();
    String typeName = testClass.getName();
  ...
  /**
   * The amount of time to wait for all clients to have contacted the server and begin running the test. "Contacted"
   * does not necessarily mean "the test has begun," e.g. for linker errors stopping the test initialization.
   */
  private long baseTestBeginTimeoutMillis;

  /**
   * The amount of time to wait for all clients to complete a single test method, in milliseconds, measured from when
   * the <i>last</i> client connects (and thus starts the test). Set by the -testMethodTimeout argument.
   */
  ...
  /**
   * The remote user agents that have connected. Populated after all user agents have connected so we can limit
   * permutations for remote tests.
   */
  private String[] remoteUserAgents;

  /**
   * What type of test we're running; Local development, local production, or remote production.
   */
  private RunStyle runStyle = null;

  /**
   * The argument passed to -runStyle. This is parsed later so we can pass in a logger.
   */
  ...
  /**
   * The time at which the current test will fail if the client has not yet started the test.
   */
  private long testBeginTimeout;

  /**
   * Timeout for individual test method. If System.currentTimeMillis() is later than this timestamp, then we need to
   * pack up and go home. Zero for "not yet set" (at the start of a test). This interval begins when the
   ...
   * 
   * @param runStyleName
   *          the argument passed to -runStyle
  ...
  private void runTestImpl(GWTTestCase testCase, Class<?> testClass, TestResult testResult)
      throws UnableToCompleteException {
    runTestImpl(testCase, testClass, testResult, 0);
  }
  ...
  private void runTestImpl(GWTTestCase testCase, Class<?> testClass, TestResult testResult,
    ...
    if (mustNotExecuteTest(getBannedPlatforms(testClass,
        testCase.getName()))) {
      return;
    }
    ...
    JUnitFatalLaunchException launchException = checkTestClassInCurrentModule(getTopLogger(),
        currentCompilationState, moduleName, testCase, testClass);
    ...
    currentModule.getRules().prepend(new RuleGenerateWith(JUnitTestCaseStubGenerator.class) {
      @Override
    ...
    if (mustRetry) {
      if (messageQueue.needsRerunning(currentTestInfo)) {
        ...
        runTestImpl(testCase, testClass, testResult, numTries);
        return;
      }
    }
