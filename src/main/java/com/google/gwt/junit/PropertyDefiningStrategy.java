...
/**
 * A {@link JUnitShell.Strategy} that will alter the module the tests are run in by defining module properties as
 * requested by annotations on the tests.
 */
public class PropertyDefiningStrategy extends GWTTestCase.BaseStrategy {
  ...
  public PropertyDefiningStrategy(TestCase testCase, Class<?> testClass) {
    this.testCase = testCase;
  ...
  private void computePropertiesMap() {
    if (properties == null) {
      ...
      try {
        ...
        if (name != null) {
          Method testMethod = (testClass != null) ? testClass.getMethod(testCase.getName()) : testCase.getClass().getMethod(testCase.getName());
