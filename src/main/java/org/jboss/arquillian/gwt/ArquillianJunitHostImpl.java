...
import org.jboss.arquillian.gwt.ArquillianJunitMessageQueue.ClientInfoExt;
...
public class ArquillianJunitHostImpl extends HybridServiceServlet implements JUnitHost {
   ...
   */
  private static ArquillianJunitMessageQueue sHost = null;

  /**
   * A maximum timeout to wait for the test system to respond with the next test. The test system should respond nearly
   * instantly if there are further tests to run, unless the tests have not yet been compiled.
   */
  ...
  /**
   * Tries to grab the GWTUnitTestShell sHost environment to communicate with the real test process.
   */
  private static synchronized ArquillianJunitMessageQueue getHost() {
    /*if (sHost == null) {
    ...
    return sHost;*/
    return ArquillianJunitMessageQueue.readFromFile();
  }

  private final Map<StrongName, Map<SymbolName, String>> symbolMaps =
      new HashMap<StrongName, Map<SymbolName, String>>();
  ...
  public TestBlock reportResultsAndGetTestBlock(
    ...
    ArquillianJunitMessageQueue host = getHost();
